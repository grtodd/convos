...
sub connect_p {
  ...
  return Mojo::Promise->reject(ucfirst $lock) if $lock;
...
sub state {
  ...
  die "Invalid state: $state"
    unless grep { $state eq $_ } qw(connected connecting queued disconnected disconnecting);
  $self->{state} = $state;
  ...
  if ($state eq 'disconnected' && $self->wanted_state eq 'connected') {
    $self->user->core->connect($self, $self->inc_reconnect_delay->reconnect_delay);
  }
